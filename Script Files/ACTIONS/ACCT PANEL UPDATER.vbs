...
BeginDialog ACCT_UPDATERone, 0, 0, 146, 150, "ACCT UPDATERone"
  ...
  EditBox 85, 45, 25, 15, MEMB_num
  EditBox 70, 65, 65, 15, worker_signature
  DropListBox 20, 115, 110, 15, " "+chr(9)+"Update"+chr(9)+"Add", add_or_update
  ButtonGroup ButtonPressed
    OkButton 5, 135, 50, 15
    CancelButton 90, 135, 50, 15
  Text 5, 70, 60, 10, "Worker Signature:"
  Text 10, 25, 50, 15, "Footer Month/Year:"
  Text 5, 50, 80, 10, "HHLD member number:"
  Text 10, 10, 50, 10, "Case Number:"
  Text 95, 30, 5, 10, "/"
  Text 5, 85, 130, 25, "Select if you are going to be updating an existing ACCT panel or adding a new ACCT panel."
EndDialog
...
BeginDialog ACCT_UPDATERthree, 0, 0, 206, 270, "ACCT UPDATERthree"
  ...
  EditBox 70, 65, 90, 15, ACCT_location
  EditBox 55, 85, 55, 15, Balance
  EditBox 145, 85, 55, 15, VERIF_type
  EditBox 45, 110, 55, 15, as_of_date
  EditBox 148, 110, 55, 15, date_recd
  EditBox 80, 130, 30, 15, penalty
  EditBox 155, 130, 25, 15, yes_no
...
DO
	...
	If add_or_update = " " then err_msg = err_msg & vbCr & "* Please select update or add an ACCT panel."
...
msgbox "hi"	
	...
	If total_amt_of_panels = "0" then script_end_procedure("No ACCT panels exist for this client. Please restart the script and select ADD an ACCT panel or select the correct HHLD member number to update. The script will now stop.")
	...
	DO
		...
		IF Balance = "" THEN err_msg = err_msg & vbCr & "You must enter the balance."
		'checks that an verification type has been selected
		IF VERIF_type = "  " THEN err_msg = err_msg & vbCr & "You must select an ACCT verification type."
		'checks if the as of date has been entered.
		IF as_of_date = "" THEN err_msg = err_msg & vbCr & "You must enter the as of date."
		'checks if the date rec'd has been entered.
		IF date_recd = "" THEN err_msg = err_msg & vbCr & "You must enter the date rec'd."
...
'redefining variables for the case note
IF add_or_update = "Add" THEN add_or_update = "added"
IF add_or_update = "Update" THEN add_or_update = "updated"
...
call write_variable_in_CASE_NOTE("***ACCT panel " & add_or_update & " for " & "Member " & MEMB_num & "***")
call write_variable_in_CASE_NOTE("* "& ACCT_type & " at " & ACCT_location & " as of " & as_of_date)
call write_variable_in_CASE_NOTE("* Date Received " & date_recd)
